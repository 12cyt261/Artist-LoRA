            ...
            with accelerator.accumulate(unet):
                ...
                if args.snr_gamma is None:
                    ...
                    if global_step < -1:
                        ...
                        z0 = model_input
                        zt = noisy_model_input
                        beta_t = torch.linspace(noise_scheduler.config.beta_start, noise_scheduler.config.beta_end, noise_scheduler.config.num_train_timesteps).to("cuda")
                        alpha_t = 1 - beta_t
                        alpha_bar_t = torch.cumprod(alpha_t, dim=0)
                        z0_hat = (zt-torch.sqrt(1-alpha_bar_t[timesteps])*model_pred)/(torch.sqrt(alpha_bar_t[timesteps]))

                        loss_1 = F.mse_loss(model_pred.float(), target.float(), reduction="mean")
                        loss_2 = cal_kl(z0.float(),z0_hat.float())
                        loss_3 = F.mse_loss(z0_hat.float(),z0.float(), reduction="mean")
            
                        loss = loss_1 + 0.0004*loss_2  - 0.0015*loss_3
                else:
